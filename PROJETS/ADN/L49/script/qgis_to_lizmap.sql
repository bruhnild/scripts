...
--- Schema : coordination
--- Vue : vue_doublons_nro

CREATE OR REPLACE VIEW coordination.vue_doublons_nro AS 
SELECT *
FROM(
WITH compte AS (SELECT id_nro, id_opp 
FROM coordination.numerisation
GROUP BY id_nro, id_opp
ORDER BY id_nro
)
SELECT   COUNT(compte.id_nro) AS nbr_doublon, id_nro
FROM     compte
GROUP BY id_nro
HAVING   COUNT(*) > 1) doublons_nro

--- Schema : coordination
--- Vue : vue_doublons_nro_all
...
SELECT   COUNT(compte.id_nro)+1 AS nbr_doublon, id_nro
FROM     compte
GROUP BY id_nro
) doublons_nro
