...
DELETE FROM pci70_edigeo_majic.geo_parcelle
WHERE ogc_fid IN (
  SELECT 
   ogc_fid
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, geo_parcelle ORDER BY ogc_fid) AS rnum
        FROM pci70_edigeo_majic.geo_parcelle ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.parcelle_info
WHERE ogc_fid IN (
  SELECT 
   ogc_fid
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY idu, geo_parcelle ORDER BY ogc_fid) AS rnum
        FROM pci70_edigeo_majic.parcelle_info ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_zoncommuni
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_zoncommuni) AS rnum
        FROM pci70_edigeo_majic.geo_zoncommuni ) t
        WHERE t.rnum > 1);
/*
-------------------------------------------------------------------------------------
...
DELETE FROM pci70_edigeo_majic.geo_batiment
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_batiment) AS rnum
        FROM pci70_edigeo_majic.geo_batiment ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_borne
WHERE geo_borne IN (
  SELECT 
   geo_borne
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_borne) AS rnum
        FROM pci70_edigeo_majic.geo_borne ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.charge_id
WHERE ogc_fid IN (
  SELECT 
   ogc_fid
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY ogc_fid) AS rnum
        FROM pci70_edigeo_majic.charge_id ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_croix
WHERE geo_croix IN (
  SELECT 
   geo_croix
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_croix) AS rnum
        FROM pci70_edigeo_majic.geo_croix ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_label
WHERE ogc_fid IN (
  SELECT 
   ogc_fid
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid ORDER BY ogc_fid) AS rnum
        FROM pci70_edigeo_majic.geo_label ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_lieudit
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_lieudit) AS rnum
        FROM pci70_edigeo_majic.geo_lieudit ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_numvoie
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_numvoie) AS rnum
        FROM pci70_edigeo_majic.geo_numvoie ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_ptcanv
WHERE geo_ptcanv IN (
  SELECT 
   geo_ptcanv
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_ptcanv) AS rnum
        FROM pci70_edigeo_majic.geo_ptcanv ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_subdfisc
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_subdfisc) AS rnum
        FROM pci70_edigeo_majic.geo_subdfisc ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_subdsect
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_subdsect) AS rnum
        FROM pci70_edigeo_majic.geo_subdsect ) t
        WHERE t.rnum > 1);
...
--- Vue : geo_subdfisc
--- Traitement : Suppression de doublons dans la table des portinos de sections cadastrales (ne garde qu'une ligne sur les 4)	
ALTER TABLE pci70_edigeo_majic.geo_subdsect ADD COLUMN ogc_fid SERIAL PRIMARY KEY;
...
DELETE FROM pci70_edigeo_majic.geo_voiep
WHERE geo_voiep IN (
  SELECT 
   geo_voiep
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_voiep) AS rnum
        FROM pci70_edigeo_majic.geo_voiep ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_tsurf
WHERE geo_tsurf IN (
  SELECT 
   geo_tsurf
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_tsurf) AS rnum
        FROM pci70_edigeo_majic.geo_tsurf ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_tronroute
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_tronroute) AS rnum
        FROM pci70_edigeo_majic.geo_tronroute ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_tpoint
WHERE geo_tpoint IN (
  SELECT 
   geo_tpoint
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_tpoint) AS rnum
        FROM pci70_edigeo_majic.geo_tpoint ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_tronfluv
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_tronfluv) AS rnum
        FROM pci70_edigeo_majic.geo_tronfluv ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_tline
WHERE geo_tline IN (
  SELECT 
   geo_tline
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_tline) AS rnum
        FROM pci70_edigeo_majic.geo_tline ) t
        WHERE t.rnum > 1);
...
DELETE FROM pci70_edigeo_majic.geo_symblim
    ...
    FROM (
      SELECT 
       ...
       ROW_NUMBER() OVER (partition BY object_rid, creat_date ORDER BY geo_symblim) AS rnum
        FROM pci70_edigeo_majic.geo_symblim ) t
        WHERE t.rnum > 1);