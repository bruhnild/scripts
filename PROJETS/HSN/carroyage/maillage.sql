...
a.*,concat(c.ccodep, c.ccocom) AS commune,
(st_area(st_intersection(a.geom, b.geom))/st_area(a.geom)) AS proportion
FROM rbal.grille_2000_polygon_2154_old a, pci70_edigeo_majic.geo_commune b
LEFT JOIN pci70_edigeo_majic.commune c ON b.geo_commune=c.geo_commune
...
a.*,concat(c.ccodep, c.ccocom) AS commune,
(st_area(st_intersection(a.geom, b.geom))/st_area(a.geom)) AS proportion
FROM rbal.grille_500_polygon_2154_old a, pci70_edigeo_majic.geo_commune b
LEFT JOIN pci70_edigeo_majic.commune c ON b.geo_commune=c.geo_commune
WHERE st_intersects(a.geom, b.geom) 
ORDER BY a.gid, proportion DESC)
