...
SELECT
	...
	,pt_gest -- REFERENCES t_organisme (or_code)
	,pt_avct -- REFERENCES l_avancement(code)
	,'A'::VARCHAR AS pt_typephy -- REFERENCES l_ptech_type_phy (code)
	,'I'::VARCHAR AS pt_typelog -- REFERENCES l_ptech_type_log (code)
	,'RF700010100001' AS pt_rf_code -- REFERENCES t_reference (rf_code)
	,type::VARCHAR AS pt_nature --  REFERENCES l_ptech_nature (code)
	,0::BOOLEAN AS pt_secu 
	,now() AS pt_creadat 
FROM orange.ft_appui_hsn_point_2154
...
SELECT
	...
	,'I'::VARCHAR AS pt_typelog -- REFERENCES l_ptech_type_log (code)
	,'RF700010100001' AS pt_rf_code -- REFERENCES t_reference (rf_code)
	,type_appui::VARCHAR AS pt_nature --  REFERENCES l_ptech_nature (code)
	,0::BOOLEAN AS pt_secu 
	,now() AS pt_creadat 
FROM orange.gespot_hsn_point_2154
