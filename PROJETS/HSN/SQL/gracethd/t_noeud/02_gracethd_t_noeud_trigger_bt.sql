...
DROP TRIGGER IF EXISTS trg_after_ed_t_noeud_bt ON avp_n070gay.boite CASCADE;
DROP FUNCTION IF EXISTS fn_after_ed_t_noeud_bt() CASCADE;

CREATE OR REPLACE FUNCTION fn_after_ed_t_noeud_bt()
RETURNS TRIGGER AS $$
	BEGIN
	CASE TG_OP
		WHEN 'INSERT' THEN
			...
			WHERE v.pt_code = concat('BT700', NEW.digt_6, NEW.digt_7, '00', to_char(NEW.id, 'FM00000'));
		...
		WHEN 'UPDATE' THEN
			IF OLD.geom IS DISTINCT FROM NEW.geom THEN 
				...
				UPDATE avp_n070gay.boite ch SET
...
CREATE TRIGGER trg_after_ed_t_noeud_bt
	AFTER INSERT OR UPDATE ON
	 avp_n070gay.boite FOR EACH ROW 
	 EXECUTE PROCEDURE fn_after_ed_t_noeud_bt();




DROP TRIGGER IF EXISTS trg_before_ed_t_noeud_bt ON avp_n070gay.boite CASCADE;
DROP FUNCTION IF EXISTS fn_before_ed_t_noeud_bt() CASCADE;

CREATE OR REPLACE FUNCTION fn_before_ed_t_noeud_bt()
RETURNS TRIGGER AS $$
	BEGIN
	CASE TG_OP
		-- WHEN 'UPDATE' THEN
			...
			UPDATE gracethd_metis.t_noeud t
			  SET 
			    ...
			    WHERE t.nd_code = (
			    	SELECT v.nd_code 
			    	FROM gracethd_metis.v_noeud v 
			    	WHERE v.pt_code = concat('BT700', OLD.digt_6, OLD.digt_7, '00', to_char(OLD.id, 'FM00000'))
	    	);
...
CREATE TRIGGER trg_before_ed_t_noeud_bt
	-- BEFORE UPDATE OR DELETE ON
	BEFORE DELETE ON
	 avp_n070gay.boite FOR EACH ROW 
	 EXECUTE PROCEDURE fn_before_ed_t_noeud_bt();